...
public class STCompiledBlock {
    ...
    public STCompiledBlock(STClass enclosingClass, STBlock blk) {
        ...
        if(blk.isMethod()) {
            List<Scope> blkt = blk.getAllNestedScopedSymbols();
            blocks = new STCompiledBlock[blkt.size()];
            for (int i = 0; i < blkt.size(); i++) {
                STBlock blockt = (STBlock) blkt.get(i);
                blocks[blockt.index] = blockt.compiledBlock;
            }
        }
